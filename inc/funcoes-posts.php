...
function upload( array $arquivo){
    ...
    $tiposValidos = [
        ...
        //verificando se o arquivo NÃO É um dos aceitos
...
function formataData( string $data):string { 
    /* pegamos a data informada, tranformamos em texto (strtotime) e depois aplicamos o dormato brasileiro (dia/mês/ANO) */
...
function busca(mysqli $conexao, string $termo):array {
    $sql = "SELECT id, titulo, data, resumo FROM 
    posts WHERE titulo LIKE '%$termo%' OR resumo LIKE '%$termo$' OR texto LIKE '%$termo%' ORDER BY data DESC ";
